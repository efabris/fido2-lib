		...
		describe("parsing attestation with " + run.functionName, function () {
			...
			describe("validateExpectations", function () {
				...
				it("throws on wrong transports value in allowCredentials element", function () {
					...
					return assert.isRejected(attResp.validateExpectations(), Error, "expected transports of allowCredentials[0] to be string with value 'usb', 'nfc', 'ble', 'cable', 'internal', 'hybrid' or null");
				});
			...
			describe("validateExtensionData", function () {
				it("returns true on validation", async function () {
					var ret = await attResp.validateExtensionData();
					assert.isTrue(ret);
					assert.isTrue(attResp.audit.journal.has("extensionData"));
				});
			});
			...
			describe("validateAudit for 'none' attestation", function () {
				it("returns on all internal checks passed", async function () {
					...
					await attResp.validateExtensionData();
