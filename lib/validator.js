		...
		if (allowCredentials != null) {
			if (!Array.isArray(allowCredentials)) {
				...
				for (const index in allowCredentials) {
					...
					} else if (allowCredentials[index].transports != null && !allowCredentials[index].transports.every(el => ["usb", "nfc", "ble", "cable", "internal", "hybrid"].includes(el))) {
						throw new Error("expected transports of allowCredentials[" + index + "] to be string with value 'usb', 'nfc', 'ble', 'cable', 'internal', 'hybrid' or null");
					}
				}
			}
		}
...
async function validateAaguid() {
	...
	if (aaguid == null && this.resultType == "assertion") {
		this.audit.journal.add("aaguid");
		return true;
	}

	if (!(aaguid instanceof ArrayBuffer)) {
		throw new Error("authnrData AAGUID is not ArrayBuffer" + (this.resultType == "assertion" ? " or null" : ""));
	}
...
async function validateCredId() {
	...
	if (credIdLen == null && this.resultType == "assertion") {
	...
	if (!(credId instanceof ArrayBuffer)) {
		throw new Error("authnrData credId should be ArrayBuffer" + (this.resultType == "assertion" ? " or null" : ""));
	}

	if (typeof credIdLen !== "number") {
		throw new Error("authnrData credIdLen should be number" + (this.resultType == "assertion" ? " or null" : "") + ", got " + typeof credIdLen);
	}
...
async function validatePublicKey() {
	...
	if (cbor == null && this.resultType == "assertion") {
	...
	if (!(cbor instanceof ArrayBuffer)) {
		throw new Error("authnrData credentialPublicKeyCose isn't of type ArrayBuffer" + (this.resultType == "assertion" ? " or null" : ""));
	}
	this.audit.journal.add("credentialPublicKeyCose");

	// jwk
	if (typeof jwk !== "object") {
		throw new Error("authnrData credentialPublicKeyJwk isn't of type Object" + (this.resultType == "assertion" ? " or null" : ""));
	}
	...
	if (typeof pem !== "string") {
		throw new Error("authnrData credentialPublicKeyPem isn't of type String" + (this.resultType == "assertion" ? " or null" : ""));
	}
...
async function validateExtensionData() {
	var extensionData = this.authnrData.get("extensionData");

	// extensionData
	if (extensionData != null && typeof extensionData !== "object") {
		throw new Error("authnrData extensionData isn't of type object");
	}
	this.audit.journal.add("extensionData");
