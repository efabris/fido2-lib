...
function parseAuthenticatorData(authnrDataArrayBuffer) {
	...
	var credIdLen;
	var credId;
	var credentialPublicKeyCose;
	var credentialPublicKeyJwk;
	var credentialPublicKeyPem;

	if (attestation) {
		aaguid = authnrDataBuf.buffer.slice(offset, offset + 16);
		offset += 16;
		credIdLen = authnrDataBuf.getUint16(offset, false);
		offset += 2;
		credId = authnrDataBuf.buffer.slice(offset, offset + credIdLen);
		offset += credIdLen;
		credentialPublicKeyCose = authnrDataBuf.buffer.slice(offset, authnrDataBuf.buffer.byteLength);
		credentialPublicKeyJwk = coseToJwk(credentialPublicKeyCose);
		credentialPublicKeyPem = jwkToPem(credentialPublicKeyJwk);
	}

	ret.set("aaguid", aaguid);
	ret.set("credIdLen", credIdLen);
	ret.set("credId", credId);
	ret.set("credentialPublicKeyCose", credentialPublicKeyCose);
	ret.set("credentialPublicKeyJwk", credentialPublicKeyJwk);
	ret.set("credentialPublicKeyPem", credentialPublicKeyPem);


	var extensionData;
	if (extensions) {
		var extensionDataCbor = authnrDataBuf.buffer.slice(offset, authnrDataBuf.buffer.byteLength);
		console.log(extensionDataCbor);
		extensionData = attestation ? cbor.decodeAllSync(extensionDataCbor)[1] : cbor.decodeFirstSync(extensionDataCbor);
		console.log(cbor.decodeAllSync(extensionDataCbor));
		console.log(extensionData);
	}

	ret.set("extensionData", extensionData);
